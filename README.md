...
- Web 控制台：`http://NAS_IP:8011`；独立设置页：`http://NAS_IP:8011/settings`。
...
- **控制台**（`/`）：运行健康、今日完成/失败/等待/最近活动、7 天完成与失败趋势、最近任务，以及立即扫描和强制扫描操作。
- **设置**（`/settings`）：目录与同步、扫描节奏、文件处理和排除目录；保存后立即生效并回到设置页。
- **任务记录**（`/stats`）：处理分类汇总与最近任务明细。
- **日志**（`/logs`）：最近日志、级别筛选和清空操作。
- **高级工具**：`/debug/candidates` 查看等待稳定的候选队列，`/debug/test-pair` 绕过扫描、稳定等待和去重，直接测试指定文件对。
